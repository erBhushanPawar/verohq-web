          ...
          <SheetHeader>
            <SheetTitle className="flex items-center">
              <WaspRouterLink to={routes.LandingPageRoute.to}>
                <span className="sr-only">VeroHQ</span>
...
function renderNavigationItems(
  ...
  const isMobile = !!setMobileMenuOpen;
  ...
  return navigationItems.map((item) => {
        ...
        <li key={item.name}>
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <button className="text-sm font-normal leading-6 text-foreground duration-300 ease-in-out hover:text-primary transition-colors flex items-center gap-1">
                {item.name}
    ...
    // Mobile menu items (including submenu items flattened)
          ...
          <ul className="ml-4 space-y-1">
            {item.submenu.map((subitem) => (
              <li key={subitem.name}>
                <ReactRouterLink
                  to={subitem.to}
                  className="block rounded-lg px-3 py-2 text-xs font-normal leading-6 text-muted-foreground hover:bg-accent hover:text-accent-foreground transition-colors"
                  onClick={() => setMobileMenuOpen(false)}
                  target={subitem.to.startsWith("http") ? "_blank" : undefined}
                >
                  {subitem.name}
    ...
    // Regular items (desktop and mobile)
    const menuStyles = cn({
